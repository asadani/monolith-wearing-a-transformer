...
instructive. Reported figures put its card-testing detection improvement at 64% on
large businesses, against a two-year effort to reach an 80% rate[^c-011]. Both
numbers are vendor-sourced and reach this report through trade press, so this is a
contrast in **disclosure**, not in verified performance[^c-017] — but Stripe names
a baseline and a specific attack class, which is the minimum an outside reader
needs to weigh a claim, and which Razorpay does not provide.
...
**On architecture, two data points arrive that the first pass lacked — and both
cut against the monolith.** Stripe positions its foundation model as an addition
to its specialist models rather than a replacement for them[^c-021]. Adyen
attempted the replacement — one global model in place of many — and reported that
it tried, failed, and would keep trying[^c-022].

That second one is the most instructive item in the corpus: a company with its own
hardware and hundreds of petabytes reporting that the single-model future is hard
even with every advantage.

**Both reach me at one remove**, through a practitioner summarising Adyen's and
Stripe's published work rather than through those publications themselves. Their
confidence is recorded as low, and neither should carry weight in print until the
primaries are obtained. Flagging that is not pedantry here — a secondhand
attribution presented as a finding is the precise failure this report opens by
documenting.
...
[^c-021]: Stripe positions its payments foundation model as an addition to its existing specialist models rather than a replacement for them.
    — *Transaction foundation models (Dwayne Gefferie)*, Dwayne Gefferie (Substack), 2025. https://dwaynegefferie.substack.com/p/transaction-foundation-models
      [T3] quote: "Stripe describes it as an addition, not a replacement"
    Note: DIRECTLY SUPPORTS THE SUPPORTING-SYSTEM POSITION, and it comes from the category's most credible builder. AT ONE REMOVE: this is a practitioner's characterisation of Stripe's published work, not Stripe's own words. Obtain Stripe's write-up before this carries weight in print.

[^c-022]: Adyen attempted to replace its many models with a single global model and reported that it tried, failed, and would keep trying.
    — *Transaction foundation models (Dwayne Gefferie)*, Dwayne Gefferie (Substack), 2025. https://dwaynegefferie.substack.com/p/transaction-foundation-models
      [T3] quote: "it tried replacing its many models with one global model and, in its own words, tried, failed, and will keep trying"
    Note: THE STRONGEST AVAILABLE EVIDENCE AGAINST THE MONOLITH, and it is an admission from a company that attempted it with hundreds of petabytes and its own hardware. AT ONE REMOVE: reported by a practitioner summarising Adyen's published Uplift work. The primary must be obtained before this becomes load-bearing -- it is exactly the kind of secondhand attribution this project exists to catch.
